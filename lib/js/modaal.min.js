	...
	!function(a) {
		...
		var o = {
			...
			create_iframe: function(url) {
				var self = this;
    			var iframeContent = null !== self.options.width || void 0 !== self.options.width || null !== self.options.height || void 0 !== self.options.height
        		? '<iframe src="' + url + '" class="modaal-iframe-elem" frameborder="0" id="modaal-iframe-elem" allowfullscreen"></iframe>'
        		: '<div class="modaal-content-container">Please specify a width and height for your iframe</div>';
    
    			// モーダルが開かれた後の処理
    			self.options.after_open = function (modal) {
        		// modaal-iframe-elem クラスが付与されたすべての iframe 要素を取得
        		var iframes = modal.find('.modaal-iframe-elem');

       	 		// 各 iframe に対してイベントハンドラを追加
        		iframes.each(function () {
            	// 右クリックイベントを無効化
            	jQuery(this).on('contextmenu', function (event) {
				...
				// モーダルを構築
				self.build_modal(iframeContent);
			},
		...
		a.fn.modaal.options = {
			...
			after_open: function() {
				$(".modaal-iframe-elem").on('contextmenu', function (e) {
				e.preventDefault();
			})},
